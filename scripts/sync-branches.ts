...
const prisma = new PrismaClient({
  datasources: {
    db: {
      url: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/bookingdb'
    }
  }
});
...
async function syncBranches() {
  try {
    ...
    for (const branch of branchesResult.rows) {
      ...
      if (!doctor) {
        console.log(`Skipping branch ${branch.id}: Doctor not found for user ${branch.user_id}`);
        skippedCount++;
        continue;
      }
      ...
      syncedCount++;
      console.log(`Synced branch: ${branch.name} (${branch.id}) for doctor: ${doctor.name}`);
    }
