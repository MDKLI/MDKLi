...
class NumericalVsNumericalAnalysis(BivariateAnalysisStrategy):
    def analyze(self, df: pd.DataFrame, feature1: str, feature2: str):
        ...
        if feature1 not in numerical_cols or feature2 not in numerical_cols:
            raise ValueError(
                "Both features must be numerical for scatter plot and regression plot.\n"
                f"This analysis is only applicable for numerical features such as:\n"
                f"{', '.join(numerical_cols)}"
            )
        else:
    ...
    def scatter_plot(self, df: pd.DataFrame, feature1: str, feature2: str):
        ...
        plt.legend(title=feature2, bbox_to_anchor=(1.02, 1), loc="upper left")
    ...
    def regression_plot(self, df: pd.DataFrame, feature1: str, feature2: str):
        ...
        sns.regplot(
            ...
            y=feature2,
            data=df,
            scatter_kws={"alpha": 0.5, "s": 60, "color": "#3b82f6"},
            line_kws={
                "color": "#ef4444",
                "linewidth": 3,
...
class CategoricalVsNumericalAnalysis(BivariateAnalysisStrategy):
    def analyze(self, df: pd.DataFrame, feature1: str, feature2: str):
        ...
        if feature1 not in categorical_cols and feature2 not in categorical_cols:
            raise ValueError(
                "At least one feature must be categorical for bar plot, box plot, and violin plot.\n"
            ...
            raise ValueError(
                "One feature must be numerical for bar plot, box plot, and violin plot.\n"
                f"This analysis is only applicable for numerical features such as:\n"
                f"{', '.join(numeric_cols)}"
            )
...
if __name__ == "__main__":
    ...
    analyzer.execute_analysis(df, "WBC", "LYMp")

    analyzer.set_strategy(CategoricalVsNumericalAnalysis())
    analyzer.execute_analysis(df, "Diagnosis", "WBC")