...
router.post(
	"/kick",
	[body("doctorId").isUUID(), body("branchId").isUUID()],
	kickDoctorFromBranch,
);

// Doctor leaves branch
router.post("/leave", [body("branchId").isUUID()], leaveBranch);

// Doctor leaves facility (all branches)
router.post("/leave-facility", [body("facilityId").isUUID()], leaveFacility);
