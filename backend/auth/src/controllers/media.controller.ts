...
export const uploadProfilePicture = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (user.role === 'doctor') {
      ...
      const clinic = await clinicRepo.findOne({ where: { user: { id: user.id } } });
      if (clinic) {
...
// Upload branch media
export const uploadBranchMedia = async (req: Request, res: Response): Promise<void> => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    res.status(400).json({ errors: errors.array() });
    return;
...
// Delete branch media
export const deleteBranchMedia = async (req: Request, res: Response): Promise<void> => {
  try {
    const user = (req as any).user as User;
    if (!user) {
      res.status(401).json({ message: 'Unauthorized' });
