...
export const completeRegistration = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    logger.info('completeRegistration called with onboardingData:', JSON.stringify(onboardingData, null, 2));
    ...
    switch (role) {
      ...
      case 'clinic_admin': {
        ...
        // Map profile data from onboardingData (sent from frontend localStorage)
        clinicProfile.clinic_name = onboardingData.clinic_name || onboardingData.facility_name || onboardingData.facilityName;
        clinicProfile.photo_url = onboardingData.photo_url;
        clinicProfile.phone_numbers = onboardingData.phone_numbers || (onboardingData.phone_number ? [onboardingData.phone_number] : []);
        ...
        logger.info('Creating clinic profile with data:', JSON.stringify(clinicProfile, null, 2));
      ...
      case 'doctor': {
        ...
        doctorProfile.has_private_practice = onboardingData.has_private_practice;
        
        doctorProfile.user = user;
        
        logger.info('Creating doctor profile with mapped data:', JSON.stringify(doctorProfile, null, 2));
      ...
      case 'pharmacy_admin': {
        ...
        // Map profile data from onboardingData (sent from frontend localStorage)
        pharmacyProfile.pharmacy_name = onboardingData.pharmacy_name || onboardingData.facility_name || onboardingData.facilityName;
        pharmacyProfile.photo_url = onboardingData.photo_url;
        pharmacyProfile.phone_numbers = onboardingData.phone_numbers || (onboardingData.phone_number ? [onboardingData.phone_number] : []);
        ...
        logger.info('Creating pharmacy profile with data:', JSON.stringify(pharmacyProfile, null, 2));
        ...
        // Publish event to RabbitMQ (after transaction commits)
        setImmediate(() => {
          publishFacilityCreated(pharmacyProfile.id, 'pharmacy').catch(err => {
            logger.error('Failed to publish facility created event:', err);
          });
        });
        break;
...
export const checkPendingRegistration = async (req: Request, res: Response): Promise<void> => {
  try {
...
async function createBranches(
  ...
  for (const branchData of branchesData) {
    ...
    if (branchData.googleMapsUrl) {
      const latLngMatch = branchData.googleMapsUrl.match(/@(-?\d+\.?\d*),(-?\d+\.?\d*)/);
      if (latLngMatch) {
