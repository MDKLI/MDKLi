		...
		switch (user.role) {
			case UserRole.PATIENT: {
				...
				const updateData: any = {
					...
					current_medications:
						req.body.currentMedications ?? profile.current_medications,
				...
				if (
					...
					req.body.emergencyContactEmail !== undefined
				) {
			...
			case UserRole.CLINIC_ADMIN: {
				...
				const updateData: any = {
					clinic_name:
						req.body.facilityName || req.body.clinicName || profile.clinic_name,
					photo_url: req.body.photoUrl || profile.photo_url,
			...
			case UserRole.DOCTOR: {
				...
				const updateData = {
					...
					gender: req.body.gender || profile.gender,
					years_of_experience:
						req.body.yearsOfExperience || profile.years_of_experience,
					has_private_practice:
						req.body.hasPrivatePractice !== undefined
							? req.body.hasPrivatePractice
							: profile.has_private_practice,
					description:
						req.body.bio || req.body.description || profile.description,
				};
				...
				if (req.body.branches) {
					...
					if (req.body.branches.length > 0 && !profile.has_private_practice) {
						...
						logger.info(
							`Doctor ${user.id} added branches, automatically setting has_private_practice to true`,
						);
					}
				}
	...
	try {
		...
		for (const branchData of branchesData) {
			...
			branch.google_maps_url =
				branchData.googleMapsUrl || branchData.google_maps_url;
			branch.phone_numbers =
				branchData.phoneNumbers || branchData.phone_numbers || [];
			branch.consultation_fee =
				branchData.consultationFee || branchData.consultation_fee;
...
export const deleteBranch = async (
	...
	try {
		...
		await queryRunner.manager.delete("doctor_branch_invitation_branches", {
			branchId,
		});
