...
const invitationRepo = () =>
	AppDataSource.getRepository(DoctorBranchInvitation);
const _invitationBranchRepo = () =>
	AppDataSource.getRepository(DoctorBranchInvitationBranch);
...
const _userRepo = () => AppDataSource.getRepository(User);
...
export const findDoctors = async (
	...
	try {
		...
		if (
			...
			facility.facility_type !== "center"
		) {
			res.status(403).json({
				message: "Only hospitals and medical centers can invite doctors",
			});
			return;
		}
		...
		const invitedDoctorIds = new Set(
			existingInvitations.map((inv) => inv.doctorId),
		);
...
export const createInvitation = async (
	...
	try {
		...
		const existingInvitation = await queryRunner.manager.findOne(
		...
		if (existingInvitation) {
			res
				.status(400)
				.json({ message: "An invitation is already pending for this doctor" });
			return;
		}
		...
		if (existingAssignments.length > 0) {
			res.status(400).json({
				message:
					"Doctor is already assigned to one or more of the selected branches",
			});
			return;
		}
		...
		for (const branchData of branches) {
			...
			if (!branch) {
				res
					.status(404)
					.json({ message: `Branch ${branchData.branchId} not found` });
				return;
			}

			const invitationBranch = queryRunner.manager.create(
...
export const getFacilityInvitations = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
...
export const getFacilityBranches = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
...
export const getDoctorFacilityBranches = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
...
export const kickDoctorFromBranch = async (
	req: Request,
	res: Response,
): Promise<void> => {
	const queryRunner = AppDataSource.createQueryRunner();
	...
	try {
		...
		if (result.affected === 0) {
			res
				.status(404)
				.json({ message: "Doctor is not assigned to this branch" });
			return;
		}
...
export const leaveBranch = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
...
export const getBranchDoctors = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
