...
export async function publishUserCreated(userId: string): Promise<void> {
	try {
		...
		const patientProfile = await patientRepo.findOne({
...
export async function publishFacilityCreated(
	facilityId: string,
	...
	try {
		...
		if (type === "clinic") {
			...
			eventData = {
				...
				facility_role:
					facility.facility_type === "pharmacy" ? "pharmacy" : "clinic",
			};
		} else {
		...
		logger.error(
			`Failed to publish facility.created event: ${facilityId}`,
			error,
		);
	}
}

export async function publishFacilityUpdated(
	facilityId: string,
	type: "clinic" | "pharmacy",
): Promise<void> {
	try {
		...
		logger.error(
			`Failed to publish facility.updated event: ${facilityId}`,
			error,
		);
	}
}

export async function publishFacilityDeleted(
	facilityId: string,
): Promise<void> {
	try {
		...
		logger.error(
			`Failed to publish facility.deleted event: ${facilityId}`,
			error,
		);
	}
}
...
export async function publishBranchCreated(
	branchId: string,
	userId: string,
): Promise<void> {
	try {
...
export async function publishBranchUpdated(
	branchId: string,
	userId: string,
): Promise<void> {
	try {
...
export async function publishInvitationAccepted(
	invitationId: string,
): Promise<void> {
	try {
		...
		logger.error(
			`Failed to publish invitation.accepted event: ${invitationId}`,
			error,
		);
	}
}

export async function publishInvitationRejected(
	invitationId: string,
): Promise<void> {
	try {
		const repo = AppDataSource.getRepository(DoctorBranchInvitation);
		...
		logger.error(
			`Failed to publish invitation.rejected event: ${invitationId}`,
			error,
		);
	}
}
...
export async function publishAllData(): Promise<void> {
	try {
		...
		logger.info(
			`✅ Resync complete: ${users.length} users, ${doctors.length} doctors, ${clinics.length} clinics, ${pharmacies.length} pharmacies`,
		);
	} catch (error) {
