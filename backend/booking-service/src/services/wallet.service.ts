...
export const walletService = {
  ...
  async creditForAppointment(recipientUserId: string, grossAmount: number, appointmentId: string) {
    const wallet = await this.getOrCreateWallet(recipientUserId)
    const net = Math.round(grossAmount * (1 - PLATFORM_CUT_PERCENT / 100))

    await prisma.$transaction([
      prisma.wallet.update({ where: { id: wallet.id }, data: { balance: { increment: net } } }),
