...
import {
	fetchSlotGenerationContext,
	generateAvailableSlots,
} from "../availability/slot-generator";
...
router.get("/doctors/:doctorId", async (req, res, next) => {
	try {
		...
		logger.info(
			`API: Returning doctor ${doctorId} with ${allBranches.length} branches (${doctor.branches.length} owned, ${assignedBranches.length} facility):`,
...
router.get("/branches/:branchId/slots", async (req, res, next) => {
	try {
		...
		// Fetch context data
		const context = await fetchSlotGenerationContext(
			branchId,
			dateMoment.toDate(),
			branch.timezone,
		);
...
router.post("/appointments", async (req, res, next) => {
	try {
		...
		if (!patient) {
			...
			logger.info(
				`Auto-created patient record for userId ${userId} on first booking`,
			);
		}
		...
		if (requestedDoctorId) {
			...
			if (assignment) {
				...
				res
					.status(400)
					.json({ error: "This doctor is not assigned to this branch" });
				return;
			}
		}
		...
		if (!resolvedDoctorId) {
			res
				.status(400)
				.json({ error: "doctorId is required to book this branch" });
			return;
		}
		...
		logger.info(
			`Appointment ${appointment.id} created as PENDING_PAYMENT for patient ${patientId}`,
		);
