...
export function generateAvailableSlots(
	...
	logger.debug(
		`Generating slots for branch ${input.branchId} on ${dateMoment.format("YYYY-MM-DD")} (day ${dayOfWeek})`,
	);

	// Step 1: Find applicable rules for this day of week
	const applicableRules = context.rules.filter(
	...
	if (dateMoment.isSame(now, "day")) {
		slots = slots.filter((slot) => {
			const slotTime = moment(
				`${dateMoment.format("YYYY-MM-DD")} ${slot.startTime}`,
			).tz(timezone);
	...
	logger.debug(
		`Generated ${slots.length} slots for ${dateMoment.format("YYYY-MM-DD")}`,
	);
...
function generateSlotsFromRule(rule: {
	...
	while (
		current.clone().add(rule.slotDurationMinutes, "minutes").isSameOrBefore(end)
	) {
		const slotStart = current.format("HH:mm");
		const slotEnd = current
			.clone()
			.add(rule.slotDurationMinutes, "minutes")
			.format("HH:mm");
...
function mergeTimeRanges(
	...
	const sorted = [...ranges].sort((a, b) => {
		return moment(`2000-01-01 ${a.start}`, "YYYY-MM-DD HH:mm").diff(
			moment(`2000-01-01 ${b.start}`, "YYYY-MM-DD HH:mm"),
		);
	});
