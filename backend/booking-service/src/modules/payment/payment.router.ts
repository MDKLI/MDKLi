...
router.get('/status/:appointmentId', async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      data: { status: appointment.status, payment: appointment.payments[0] || null },
    })
  } catch (error) {
    next(error)
  }
...
router.post('/webhook', async (req, res, next) => {
  try {
    ...
    if (!transaction) {
      logger.error(`Webhook received for unknown paymob order ${paymobOrderId}`)
    ...
    if (success) {
      const recipientUserId = appointment.branch.ownerUserId || appointment.doctor.userId
      await walletService.creditForAppointment(recipientUserId, transaction.amount, appointment.id)
