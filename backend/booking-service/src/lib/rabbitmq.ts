...
export class RabbitMQClient {
  ...
  private async setupQueues(): Promise<void> {
    ...
    const queue = await this.channel.assertQueue('booking-service-events', { durable: true })
  ...
  private async handleMessage(msg: any): Promise<void> {
    ...
    try {
      ...
      logger.info(`Received RabbitMQ event: ${routingKey}`, { userId: content.id })
...
private async handleDoctorEvent(data: any): Promise<void> {
  try {
    const { id, user_id, full_name, specialty, description, photo_url } = data
  ...
  private async handleDoctorProfileUpdate(data: any): Promise<void> {
    try {
      const { userId, fullName, specialty, title, bio, photoUrl, hasPrivatePractice } = data
