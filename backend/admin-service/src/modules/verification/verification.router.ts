...
verificationRouter.patch('/doctors/:id/verify', async (req: AuthedRequest, res) => {
  try {
    const response = await callAuthService(`/admin/doctors/${req.params.id}/verify`, req)
    if (!response.ok) {
    ...
    await prisma.doctorVerification.update({
      ...
      data: { verificationStatus: 'verified', reviewedBy: req.user!.userId, reviewedAt: new Date() },
    })
    await prisma.auditLog.create({
      data: { adminId: req.user!.userId, action: 'doctor.verify', targetId: req.params.id },
    })
...
verificationRouter.patch('/doctors/:id/reject', async (req: AuthedRequest, res) => {
  try {
    const response = await callAuthService(`/admin/doctors/${req.params.id}/reject`, req)
    if (!response.ok) {
      res.status(response.status).json(await response.json())
      return
    }
    ...
    await prisma.auditLog.create({
      data: { adminId: req.user!.userId, action: 'doctor.reject', targetId: req.params.id, metadata: req.body },
    })
...
verificationRouter.get('/facilities/:category', async (req: AuthedRequest, res) => {
...
verificationRouter.patch('/facilities/:id/verify', async (req: AuthedRequest, res) => {
  try {
    const facility = await prisma.facilityVerification.findUnique({ where: { id: req.params.id } })
    if (!facility) {
      res.status(404).json({ error: 'Facility not found in cache' })
      return
    }
    const response = await callAuthService(`/admin/facilities/${req.params.id}/verify`, req, {
    ...
    await prisma.facilityVerification.update({
      where: { id: req.params.id },
      data: { status: 'verified', reviewedBy: req.user!.userId, reviewedAt: new Date() },
    })
    await prisma.auditLog.create({
      data: { adminId: req.user!.userId, action: 'facility.verify', targetId: req.params.id },
    })
    res.json({ data: { verified: true } })
  } catch (error) {
...
verificationRouter.patch('/facilities/:id/reject', async (req: AuthedRequest, res) => {
  try {
    const facility = await prisma.facilityVerification.findUnique({ where: { id: req.params.id } })
    if (!facility) {
      res.status(404).json({ error: 'Facility not found in cache' })
    ...
    const response = await callAuthService(`/admin/facilities/${req.params.id}/reject`, req, {
      type: facility.facilityType,
    })
    if (!response.ok) {
      res.status(response.status).json(await response.json())
    ...
    await prisma.auditLog.create({
      data: { adminId: req.user!.userId, action: 'facility.reject', targetId: req.params.id, metadata: req.body },
    })
    res.json({ data: { rejected: true } })
  } catch (error) {
...
verificationRouter.patch('/users/:userId/block', async (req: AuthedRequest, res) => {
  try {
    const response = await callAuthService(`/admin/users/${req.params.userId}/block`, req)
    if (!response.ok) {
      res.status(response.status).json(await response.json())
      return
    }
    ...
    await prisma.auditLog.create({
      data: { adminId: req.user!.userId, action: 'user.block', targetId: req.params.userId },
    })
...
verificationRouter.patch('/users/:userId/unblock', async (req: AuthedRequest, res) => {
  try {
    const response = await callAuthService(`/admin/users/${req.params.userId}/unblock`, req)
    if (!response.ok) {
      res.status(response.status).json(await response.json())
      return
    }
    ...
    await prisma.auditLog.create({
      data: { adminId: req.user!.userId, action: 'user.unblock', targetId: req.params.userId },
    })
...
verificationRouter.get('/blocked', async (req: AuthedRequest, res) => {
  ...
  const data = await Promise.all(
    users.map(async (u) => {
      const doctor = await prisma.doctorVerification.findFirst({ where: { userId: u.id } })
      const facility = doctor ? null : await prisma.facilityVerification.findFirst({ where: { userId: u.id } })

      const displayName = doctor?.fullName || facility?.name || u.username || u.email || u.id
      const accountType = doctor ? 'doctor' : facility ? facility.facilityType : u.role
