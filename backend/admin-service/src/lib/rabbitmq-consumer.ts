...
export class AdminRabbitMQConsumer {
  ...
  async connect(): Promise<void> {
    ...
    const queue = await this.channel.assertQueue('admin-service-events', { durable: true })
  ...
  private async syncUser(data: any) {
    const { id, username, email, role, is_suspended, blocked_at, deleted_at } = data
    if (!id) return
    await prisma.user.upsert({
      ...
      update: {
        ...
        blockedAt: blocked_at ? new Date(blocked_at) : blocked_at === null ? null : undefined,
        deletedAt: deleted_at ? new Date(deleted_at) : deleted_at === null ? null : undefined,
      },
      create: {
        id,
  ...
  private async syncDoctor(data: any) {
    const { id, user_id, full_name, specialty, photo_url, verification_status } = data
    if (!id) return
  ...
  private async syncAppointment(data: any) {
    const { id, doctorId, branchId, patientId, status, date, consultationFee } = data
    if (!id) return
    await prisma.appointmentSummary.upsert({
      where: { id },
      update: { status: status ?? undefined, consultationFee: consultationFee ?? undefined },
      create: {
        id,
        doctorId,
        branchId,
