...
import { MediaService, MEDIA_TYPE_LIMITS } from '../../services/media.service'
...
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: MAX_POSSIBLE_SIZE } })

export const mediaRouter = Router()
mediaRouter.use(requireAuth)

// POST /media/upload/:roomId  (multipart form: file, type)
mediaRouter.post('/upload/:roomId', upload.single('file'), async (req: AuthedRequest, res) => {
  try {
