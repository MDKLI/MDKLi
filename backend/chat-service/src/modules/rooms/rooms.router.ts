...
roomsRouter.get('/', async (req: AuthedRequest, res) => {
  try {
    ...
    const otherIds = rooms.map((r) => (r.userAId === userId ? r.userBId : r.userAId))
    const otherUsers = await prisma.user.findMany({ where: { id: { in: otherIds } } })
    const userMap = new Map(otherUsers.map((u) => [u.id, u]))
    const unreadCounts = await prisma.message.groupBy({
      by: ['roomId'],
      where: { roomId: { in: rooms.map((r) => r.id) }, senderId: { not: userId }, readAt: null },
      _count: { id: true },
    })
    ...
    const result = rooms.map((r) => {
      ...
      return {
        id: r.id,
        otherUser: userMap.get(otherId) || { id: otherId, name: 'Unknown', photoUrl: null, about: null },
...
roomsRouter.post('/:id/block', async (req: AuthedRequest, res) => {
  try {
    ...
    getIO().to(roomId).emit('room_blocked', { roomId, blockedBy: req.user!.userId })
