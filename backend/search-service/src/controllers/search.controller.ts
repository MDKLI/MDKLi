	...
	try {
		...
		if (query && query !== "*") {
			qb.andWhere(
				new Brackets((qb2) => {
					qb2
						.where("doctor.full_name ILIKE :query", { query: `%${query}%` })
						.orWhere("doctor.specialty ILIKE :query", { query: `%${query}%` })
						.orWhere("doctor.description ILIKE :query", {
							query: `%${query}%`,
						});
				}),
			);
			...
			filteredDoctors = filteredDoctors.filter((doctor) => {
				...
				if (location_type === "hospital") {
					...
					return branches.some(
						(branch: any) => branch.branch_type === "hospital",
					);
...
export const searchFacilities = async (
	...
	try {
		...
		if (
			otherFilters.facility_role &&
			typeof otherFilters.facility_role === "string"
		) {
			qb.where("facility.facility_role = :facility_role", {
				facility_role: otherFilters.facility_role,
			});
		}
		...
		if (
			otherFilters.facility_type &&
			typeof otherFilters.facility_type === "string"
		) {
			qb.andWhere("facility.facility_type = :facility_type", {
				facility_type: otherFilters.facility_type,
			});
		}
...
export const getSuggestions = async (
	...
	try {
		...
		if (!type || type === "facility") {
			const facilitiesIndex = meiliClient.index(FACILITIES_INDEX);
			const facilitiesResult = await facilitiesIndex.search(query, {
				limit: 5,
			});
			suggestions.push(
...
const ALL_TITLES = ["professor", "lecturer", "consultant", "specialist"];
...
export const getFilters = async (
	...
	try {
		...
		const doctors = await doctorRepo().find({
			select: ["specialty", "title", "branches"],
		});
		const facilities = await facilityRepo().find({
			select: ["facility_type", "city", "area"],
		});
...
function buildFacilityFilters(filters: any): string | undefined {
	...
	if (filters.facility_type)
		conditions.push(`facility_type = "${filters.facility_type}"`);
	if (filters.facility_role)
		conditions.push(`facility_role = "${filters.facility_role}"`);
