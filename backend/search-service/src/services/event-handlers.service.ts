...
import { syncDoctorToMeilisearch, syncFacilityToMeilisearch, deleteDoctorFromMeilisearch, deleteFacilityFromMeilisearch } from './sync.service'
...
export async function handleDoctorUpdated(data: any): Promise<void> {
  try {
    ...
    existing.years_of_experience = data.years_of_experience?.toString() || existing.years_of_experience
    ...
    existing.photo_url = data.photo_url !== undefined ? data.photo_url : existing.photo_url
    ...
    existing.verification_status = data.verification_status || existing.verification_status
...
export async function handleFacilityCreated(data: any): Promise<void> {
  try {
    ...
    const facility = facilityRepo().create({
      id: data.id,
      user_id: data.user_id,
      facility_name: data.facility_name || data.clinic_name || 'Unknown Facility',
      ...
      facility_role: data.facility_role || (data.facility_type === 'pharmacy' ? 'pharmacy' : 'clinic'),
      branches: data.branches || [],
    })
...
export async function handleFacilityUpdated(data: any): Promise<void> {
  try {
    ...
    existing.address = data.address !== undefined ? data.address : existing.address
    existing.city = data.city !== undefined ? data.city : existing.city
    existing.area = data.area !== undefined ? data.area : existing.area
  ...
  try {
    ...
    if (branchIndex >= 0) {
      branches[branchIndex] = {
        ...
        address: data.address !== undefined ? data.address : branches[branchIndex].address,
        phone_numbers: data.phone_numbers !== undefined ? data.phone_numbers : branches[branchIndex].phone_numbers,
        consultation_fee: data.consultation_fee !== undefined ? data.consultation_fee : branches[branchIndex].consultation_fee,
      }
