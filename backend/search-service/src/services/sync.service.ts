...
export async function syncDoctorToMeilisearch(doctor: SearchableDoctor): Promise<void> {
  try {
...
export async function deleteDoctorFromMeilisearch(doctorId: string): Promise<void> {
  try {
    const index = meiliClient.index(DOCTORS_INDEX)
    await index.deleteDocument(doctorId)
    logger.debug(`Deleted doctor ${doctorId} from Meilisearch`)
  } catch (error) {
    logger.error(`Failed to delete doctor ${doctorId} from Meilisearch:`, error)
    throw error
  }
}

// Sync a single facility to Meilisearch
export async function syncFacilityToMeilisearch(facility: SearchableFacility): Promise<void> {
  try {
    ...
    logger.error(`Failed to sync facility ${facility.id} to Meilisearch:`, error)
    throw error
  }
}

// Delete facility from Meilisearch
export async function deleteFacilityFromMeilisearch(facilityId: string): Promise<void> {
  try {
    const index = meiliClient.index(FACILITIES_INDEX)
    await index.deleteDocument(facilityId)
    logger.debug(`Deleted facility ${facilityId} from Meilisearch`)
  } catch (error) {
    logger.error(`Failed to delete facility ${facilityId} from Meilisearch:`, error)
    throw error
  }
}
