...
export async function migrateDoctorsFromAuth() {
    ...
    `);
		...
		await client.end();
		logger.info(
			`✅ Migrated ${result.rows.length} doctors to searchable_doctors`,
		);
	} catch (error) {
...
export async function migrateFacilitiesFromAuth() {
    ...
    `);
		...
		logger.info(
			`Fetched ${result.rows.length} facilities and ${branchesResult.rows.length} branches from auth database`,
		);
		...
		for (const row of result.rows) {
			...
			const facilityRole =
				row.facility_type === "pharmacy" ? "pharmacy" : "clinic";
		...
		await client.end();
		logger.info(
			`✅ Migrated ${result.rows.length} facilities to searchable_facilities`,
		);
	} catch (error) {
