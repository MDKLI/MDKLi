...
const CHAT_API_URL =
	import.meta.env.VITE_CHAT_API_URL || "http://localhost:3005";
...
export const chatApi = {
	...
	openRoomWith: (targetUserId: string) =>
		chatFetch<{ id: string }>(`/rooms/with/${targetUserId}`, {
			method: "POST",
		}),

	blockRoom: (roomId: string) =>
		chatFetch<{ blocked: boolean }>(`/rooms/${roomId}/block`, {
			method: "POST",
		}),

	unblockRoom: (roomId: string) =>
		chatFetch<{ blocked: boolean }>(`/rooms/${roomId}/unblock`, {
			method: "POST",
		}),
	...
	uploadMedia: (
		...
		return chatFetch<{ url: string; size: number; type: string }>(
			`/media/upload/${roomId}`,
			{
				method: "POST",
				body: formData,
