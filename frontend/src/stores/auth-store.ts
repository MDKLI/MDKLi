...
interface AuthState {
	auth: {
		...
		startRegistration: (userData: Record<string, unknown>) => Promise<{
			success: boolean;
			error?: string;
			pendingToken?: string;
			role?: string;
...
export const useAuthStore = create<AuthState>()((set, get) => {
	...
	return {
		auth: {
			...
			startRegistration: async (userData: Record<string, unknown>) => {
			...
			completeRegistration: async (onboardingData: Record<string, unknown>) => {
