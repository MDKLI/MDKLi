...
type RawFacilityResult = Omit<FacilityDetails, "username">;
...
function FacilityDetailPage() {
	...
	const fetchBranchesWithDoctors = useCallback(
		async (facilityUserId: string) => {
			try {
				const result =
					await bookingApi.getFacilityBranchesForPublic(facilityUserId);
				if (result.error || !result.data?.success) {
					setBranches([]);
					return;
				}
				setBranches(result.data.data as FacilityBranch[]);
			} catch {
				setBranches([]);
			}
		},
	...
	const fetchFacilityDetails = useCallback(async () => {
		try {
			...
			const found = data.data?.find(
				(f: RawFacilityResult) =>
					f.facility_name?.toLowerCase().replace(/\s+/g, "-") ===
	...
	}, [username, fetchBranchesWithDoctors]);
		...
		return (
			<div className="flex flex-col items-center justify-center min-h-screen gap-4">
				...
				<p className="text-muted-foreground">
					The hospital or medical center you're looking for doesn't exist.
				</p>
				<Link to="/search">
					<Button>
	...
	return (
		<div className="container mx-auto px-4 py-8 max-w-6xl">
			<Link to="/search" search={{ mode: "facilities" }}>
				...
				<CardContent className="p-6">
					<div className="flex flex-col md:flex-row gap-6">
						<Avatar className="h-32 w-32 rounded-xl flex-shrink-0">
							<AvatarImage
								src={facility.photo_url || undefined}
							...
							{facility.status === "verified" ? (
								<div className="flex items-center gap-3 text-green-600">
									...
									<div>
										...
										<p className="text-sm">
											This facility has been verified by our team
										</p>
									</div>
								</div>
							) : (
