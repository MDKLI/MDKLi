...
import { MapPin, Plus, X, ChevronLeft, ChevronRight, Users, Loader2 } from 'lucide-react'
...
const generateTimeOptions = () => {
  ...
  for (let hour = 0; hour < 24; hour++) {
    for (let minute = 0; minute < 60; minute += 30) {
      ...
      times.push({ value: timeValue, label: `${displayHour}:${minStr} ${ampm}` })
    }
  }
  return times
}
...
export function FacilityAvailability() {
  ...
  const loadFacilityBranches = async () => {
    try {
      const profileResult: any = await profileApi.getProfile()
      const facilityId = profileResult?.data?.id
      if (!facilityId) { setBranches([]); return }

      const result: any = await invitationApi.getFacilityBranches(facilityId)
      const list = result?.data?.data || []
      setBranches(list.map((b: any) => ({ id: b.id, name: b.name, city: b.city, area: b.area })))
    } catch {
  ...
  const loadBranchDoctors = async (branchId: string) => {
    setDoctorsLoading(true)
    try {
      const result: any = await invitationApi.getBranchDoctors(branchId)
      const list = result?.data?.data || []
      setDoctors(list.map((d: any) => ({ id: d.id, fullName: d.fullName, specialty: d.specialty })))
    } catch {
  ...
  const loadSchedule = async (branchId: string, doctorId: string) => {
    try {
      const result = await bookingApi.getDoctorBranchAvailability(branchId, doctorId)
      ...
      const schedule: WeeklySchedule = {}
      DAYS_OF_WEEK.forEach(day => { schedule[day.value] = { enabled: false, slots: [] } })
  ...
  const loadBlockOutDates = async (branchId: string, doctorId: string) => {
    try {
      const result = await bookingApi.getDoctorBranchOverrides(branchId, doctorId)
      if (result.error || !result.data?.success) { setBlockOutDates([]); return }

      const mapped = (result.data.data || [])
        .filter((o: any) => o.type === 'BLOCK')
        .map((o: any) => ({ id: o.id, date: moment(o.date).format('YYYY-MM-DD'), reason: o.reason || undefined }))
  ...
  const toggleDay = (dayValue: number) => {
    setWeeklySchedule(prev => ({ ...prev, [dayValue]: { ...prev[dayValue], enabled: !prev[dayValue].enabled } }))
  }

  const addTimeSlot = (dayValue: number) => {
    setWeeklySchedule(prev => ({
      ...prev,
      [dayValue]: { ...prev[dayValue], slots: [...prev[dayValue].slots, { id: `${dayValue}-${Date.now()}`, startTime: '09:00', endTime: '17:00' }] }
    }))
  }
  ...
  const removeTimeSlot = (dayValue: number, slotId: string) => {
    setWeeklySchedule(prev => ({
      ...prev,
      [dayValue]: { ...prev[dayValue], slots: prev[dayValue].slots.filter(s => s.id !== slotId) }
    }))
  }
  ...
  const updateTimeSlot = (dayValue: number, slotId: string, field: 'startTime' | 'endTime', value: string) => {
    setWeeklySchedule(prev => ({
      ...prev,
      [dayValue]: { ...prev[dayValue], slots: prev[dayValue].slots.map(s => s.id === slotId ? { ...s, [field]: value } : s) }
    }))
  }
    ...
    try {
      ...
      for (const day of DAYS_OF_WEEK) {
        ...
        for (const slot of daySchedule.slots) {
          ...
          rules.push({ dayOfWeek: day.value, startTime: slot.startTime, endTime: slot.endTime, slotDurationMinutes: 30 })
        }
      }

      const result = await bookingApi.replaceDoctorBranchAvailability(selectedBranch, { doctorId: selectedDoctor, rules })
      if (result.error || !result.data?.success) {
  ...
  const addBlockOutDates = async () => {
    if (!selectedBranch || !selectedDoctor || selectedBlockOutDates.length === 0) return
    try {
      ...
      for (const date of uniqueDates) {
        await bookingApi.createDoctorBranchOverride(selectedBranch, { date, type: 'BLOCK', doctorId: selectedDoctor })
      }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">Facility Availability</h2>
      ...
      <Card>
        ...
        <CardContent>
          {branches.length === 0 ? (
            <p className="text-muted-foreground text-center py-6">No branches found.</p>
          ) : (
                ...
                {branches.map(branch => (
                  <SelectItem key={branch.id} value={branch.id}>
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground">{branch.city}, {branch.area}</span>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <CardContent>
            {doctorsLoading ? (
              ...
              <p className="text-muted-foreground text-center py-6">
                No doctors assigned to this branch yet. Invite doctors from the Invitations page.
              </p>
            ) : (
                  ...
                  return (
                    <div key={day.value} className="flex items-start gap-3 py-3 border-b border-border/50 last:border-0">
                      <div className="flex items-center gap-3 w-28 pt-1">
                        <Switch checked={daySchedule.enabled} onCheckedChange={() => toggleDay(day.value)} />
                      ...
                      <div className="flex-1 space-y-2">
                        {daySchedule.enabled && daySchedule.slots.map((slot, slotIndex) => (
                          <div key={slot.id} className="flex items-center gap-2">
                            <Select value={slot.startTime} onValueChange={(v) => updateTimeSlot(day.value, slot.id, 'startTime', v)}>
                              <SelectTrigger className="w-[100px] h-8 text-xs"><SelectValue /></SelectTrigger>
                              <SelectContent>
                                {TIME_OPTIONS.map(time => <SelectItem key={time.value} value={time.value}>{time.label}</SelectItem>)}
                              </SelectContent>
                            </Select>
                            <span className="text-sm text-muted-foreground">to</span>
                            <Select value={slot.endTime} onValueChange={(v) => updateTimeSlot(day.value, slot.id, 'endTime', v)}>
                              <SelectTrigger className="w-[100px] h-8 text-xs"><SelectValue /></SelectTrigger>
                              <SelectContent>
                                {TIME_OPTIONS.map(time => <SelectItem key={time.value} value={time.value}>{time.label}</SelectItem>)}
                              </SelectContent>
                            ...
                            {(daySchedule.slots.length > 1 || slotIndex > 0) && (
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => removeTimeSlot(day.value, slot.id)}>
                                <X className="h-3 w-3" />
                              </Button>
                            )}
                            {daySchedule.slots.length === 1 && slotIndex === 0 && <div className="h-7 w-7" />}
                          </div>
                        ))}
                      </div>

                      <div className="w-10 flex justify-end pt-1">
                        <Button variant="ghost" size="icon" className="h-7 w-7" disabled={!daySchedule.enabled} onClick={() => addTimeSlot(day.value)}>
              ...
              <CardContent className="p-6 space-y-4">
                <Dialog open={isBlockOutDialogOpen} onOpenChange={setIsBlockOutDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-[400px]">
                    <DialogHeader><DialogTitle>Select blockout date(s)</DialogTitle></DialogHeader>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="font-medium">{blockOutCalendarMonth.format('MMMM YYYY')}</span>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setBlockOutCalendarMonth(prev => prev.clone().subtract(1, 'month'))}>
                            <ChevronLeft className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setBlockOutCalendarMonth(prev => prev.clone().add(1, 'month'))}>
                      ...
                      <div className="grid grid-cols-7 gap-1 text-center">
                        {weekDays.map(day => <div key={day} className="text-xs text-muted-foreground py-1">{day}</div>)}
                      </div>
                      ...
                      <Button className="w-full" onClick={addBlockOutDates} disabled={selectedBlockOutDates.length === 0}>
                        Add date(s)
                      </Button>
                    </div>
                  </DialogContent>
                ...
                {blockOutDates.length > 0 && (
                  <div className="space-y-2">
                    {blockOutDates
                      ...
                      .map(blockOut => (
                        <div key={blockOut.id} className="flex items-center justify-between py-2 px-3 bg-muted rounded-md">
                          <span className="text-sm">{moment(blockOut.date).format('ddd, MMM D, YYYY')}</span>
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => removeBlockOutDate(blockOut.id)}>
                            <X className="h-3 w-3" />
                          </Button>
                        </div>
                      ))}
