...
import { Wallet, CreditCard, ArrowDownToLine, Trash2 } from 'lucide-react'
...
function PaymentContent() {
  ...
  const load = async () => {
    try {
      ...
      setData(wallet)
      if (wallet.cards.length && !selectedCardId) setSelectedCardId(wallet.cards[0].id)
    } catch {
  ...
  if (loading) return <div className="p-6 text-sm text-muted-foreground">Loading wallet...</div>
  if (!data) return null
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Sandbox demo — 90% of every paid booking is credited here, 10% platform fee.
            </CardDescription>
          </div>
          <Badge variant="secondary">Sandbox</Badge>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-3xl font-bold">{data.balance.toLocaleString()} EGP</div>
          <Button onClick={handleWithdraw} disabled={withdrawing || data.balance <= 0 || !selectedCardId}>
            <ArrowDownToLine className="h-4 w-4 mr-2" />
            {withdrawing ? 'Withdrawing...' : `Withdraw ${data.balance.toLocaleString()} EGP`}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Fake card numbers only — sandbox demo, no real charges.</CardDescription>
          </div>
          <Button variant="outline" size="sm" onClick={() => setShowAddCard((v) => !v)}>
        ...
        <CardContent className="space-y-4">
          ...
          {data.cards.length === 0 && !showAddCard && (
            <p className="text-sm text-muted-foreground">No payout cards yet. Add one to enable withdrawals.</p>
          )}
          ...
          {data.cards.map((card) => (
            ...
            >
              ...
              <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleRemoveCard(card.id) }}>
                ...
                <div key={tx.id} className="flex items-center justify-between border-b py-2 text-sm">
                  <div>
                    <span className={tx.type === 'CREDIT' ? 'text-green-600' : 'text-red-600'}>
                      {tx.type === 'CREDIT' ? '+' : '-'}{tx.amount} EGP
                    </span>
                    <span className="text-muted-foreground ml-2">{tx.note}</span>
                  </div>
