...
import { DollarSign, Loader2, MapPin, UserPlus } from "lucide-react";
...
export function InviteDoctorPage() {
	...
	const handleSendInvitation = async () => {
		...
		const branchesToInvite = selectedBranches.filter(
			(sb) => sb.selected && sb.consultationFee > 0,
		);
		...
		setIsSending(true);
		try {
			const result: ActionResult = await invitationApi.createInvitation({
