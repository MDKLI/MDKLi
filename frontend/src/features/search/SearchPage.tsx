...
import { Search, MapPin, Stethoscope, Building2, Filter, X, CheckCircle, Clock, ChevronRight, Award } from 'lucide-react'
...
export function SearchPage() {
  ...
  const [searchMode, setSearchMode] = useState<'doctors' | 'pharmacy'>('doctors')
  
  // Read mode from localStorage on mount and when visibility changes (e.g., when user comes back)
  useEffect(() => {
    const savedMode = localStorage.getItem('searchMode') as 'doctors' | 'pharmacy' | null
    if (savedMode) {
  ...
  const performSearch = async () => {
    ...
    try {
      if (searchMode === 'doctors') {
        ...
        // Pharmacy search
        const searchFilters: Record<string, any> = { facility_role: 'pharmacy' }
        if (filters.city) searchFilters.city = filters.city
        if (filters.area) searchFilters.area = filters.area
        
        const searchQuery = query.trim() || '*'
        const data = await searchApi.searchFacilities(searchQuery, searchFilters)
  ...
  const totalResults = searchMode === 'doctors' ? doctors.length : pharmacies.length
    ...
    <div className="flex-1 h-full overflow-auto">
        ...
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex gap-4 items-center max-w-2xl mx-auto">
            <div className="relative flex-1">
              ...
              <Input
                type="search"
                placeholder={searchMode === 'doctors' ? "Search doctors by name, specialty..." : "Search pharmacies by name, location..."}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          setFilters({ ...filters, locationType: value as Filters['locationType'] })
                        }}
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          setFilters({ ...filters, specialty: value === 'all_specialties' ? undefined : value })
                        }}
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          setFilters({ ...filters, title: value === 'all_titles' ? undefined : value })
                        }}
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          setFilters({ ...filters, gender: value === 'any_gender' ? undefined : value })
                        }}
                      >
                        <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      setFilters({ ...filters, city: value === 'all_cities' ? undefined : value, area: undefined })
                    }}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      setFilters({ ...filters, area: value === 'all_areas' ? undefined : value })
                    }}
                    disabled={!filters.city || filters.city === 'all_cities'}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={filters.city && filters.city !== 'all_cities' ? "All areas" : "Select city first"} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all_areas">All areas</SelectItem>
                      {filters.city && availableFilters.cities
                        .find(c => c.id === filters.city)?.areas
                        .map((a: string) => (
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-4 text-sm text-muted-foreground">
          {loading ? 'Searching...' : `${totalResults} ${searchMode === 'doctors' ? 'doctors' : 'pharmacies'} found`}
        </div>
        ...
        {!loading && totalResults === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">No {searchMode} found</h3>
...
function DoctorCard({ doctor }: { doctor: Doctor }) {
  ...
  const username = doctor.full_name?.toLowerCase().replace(/\s+/g, '-') || doctor.id
  ...
  return (
    <Link to={`/dr/${encodeURIComponent(username)}` as any}>
      <Card className="hover:shadow-md transition-shadow cursor-pointer h-full">
        <CardContent className="p-4">
          <div className="flex items-start gap-4">
            <Avatar className="h-16 w-16 rounded-lg">
              <AvatarImage src={doctor.photo_url || undefined} className="rounded-lg" />
          ...
          <div className="space-y-2 text-sm">
            ...
            {branchLocation && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="font-medium text-foreground">{branchLocation}</span>
              </div>
            )}
...
function PharmacyCard({ pharmacy }: { pharmacy: Pharmacy }) {
  ...
  const username = pharmacy.facility_name?.toLowerCase().replace(/\s+/g, '-') || pharmacy.id
  
  return (
    <Link to={`/ph/${encodeURIComponent(username)}` as any}>
      <Card className="hover:shadow-md transition-shadow cursor-pointer h-full">
        <CardContent className="p-4">
          <div className="flex items-start gap-4">
            <Avatar className="h-16 w-16 rounded-lg">
              <AvatarImage src={pharmacy.photo_url || undefined} className="rounded-lg" />
              <AvatarFallback className="text-lg rounded-lg bg-blue-500 text-white">
                {pharmacy.facility_name?.charAt(0).toUpperCase() || 'P'}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-base truncate">{pharmacy.facility_name}</h3>
              <div className="mt-1">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="font-medium text-foreground">
                {[pharmacy.area, pharmacy.city].filter(Boolean).join(', ') || 'Location not specified'}
              </span>
            </div>
