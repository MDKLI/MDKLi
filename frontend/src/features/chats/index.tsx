...
export function Chats() {
	...
	const loadRooms = useCallback(async () => {
		...
		if (result.data) {
			...
			const unread = result.data.filter(
				(r) =>
					r.lastMessage &&
					!r.lastMessage.readAt &&
					r.lastMessage.senderId !== myId,
			).length;
			setUnreadCount(unread);
		}
	}, [auth.user?.id, setUnreadCount]);
	...
	useEffect(() => {
		...
		const unsubscribeBlocked = onRoomBlocked((data) => {
			...
			setRooms((prev) =>
				prev.map((r) =>
					r.id === data.roomId ? { ...r, blockedBy: data.blockedBy } : r,
				),
			);
		});
	...
	}, [selectedRoomId, auth.user?.id, loadMessages, loadRooms]);

	// biome-ignore lint/correctness/useExhaustiveDependencies: intentionally re-running whenever `messages` changes to auto-scroll, even though it isn't read in the body
	useEffect(() => {
	...
	async function handleMediaSelect(
		...
		try {
			...
			emitSendMessage(
				...
				(response) => {
					if (!response.ok)
						toast.error(response.error || "Failed to send media");
				},
			);
		} finally {
			setUploading(false);
	...
	if (isBlocked) {
		...
		textboxMessage =
			"You've reached your 2-message limit, waiting for a reply.";
	}
								...
								<Fragment key={room.id}>
									...
									>
										<div className="flex w-full gap-2">
											<Avatar>
												<AvatarImage
													src={room.otherUser.photoUrl || undefined}
													alt={room.otherUser.name}
												/>
												<AvatarFallback>
													{getDisplayNameInitials(room.otherUser.name)}
												</AvatarFallback>
											</Avatar>
												...
												{groupedMessages[key].map((msg) => (
													...
													>
														...
														{msg.type === "video" && msg.mediaUrl && (
															// biome-ignore lint/a11y/useMediaCaption: user-uploaded chat media has no caption track available
															<video
																src={msg.mediaUrl}
																controls
																className="max-w-full rounded-md"
									...
									<div className="flex flex-1 items-center gap-2 rounded-md border border-input bg-card px-2 py-1 focus-within:ring-1 focus-within:ring-ring focus-within:outline-hidden lg:gap-4">
										...
										<label className="flex-1">
											...
											<input
												type="text"
												placeholder={
													textboxDisabled ? "" : "Type your messages..."
												}
