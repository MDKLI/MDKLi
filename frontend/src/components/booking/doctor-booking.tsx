...
export function DoctorBooking({ doctorId, branches }: DoctorBookingProps) {
  ...
  const fetchAvailability = async () => {
    ...
    setLoading(true)
    try {
      const response = await bookingApi.getDoctorAvailabilityForBooking(doctorId, selectedBranch.id)
                      ...
                      <div className="flex items-start gap-3 text-left">
                            ...
                            return (
                              <div className="h-10 w-10 rounded-lg flex-shrink-0 overflow-hidden bg-muted">
                                <img 
                                  ...
                                  onError={(e) => {
                                    console.log('Image failed to load:', media[0]);
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {[branch.area, branch.city].filter(Boolean).join(', ') || 'No location info'}
                          </p>
                          {branch.address && (
                          ...
                          {/* Phone */}
                          {(() => {
                            const phones = branch.phone_numbers || branch.phoneNumbers;
                          ...
                          {/* Fee */}
                          {(() => {
                            const fee = branch.consultation_fee || branch.consultationFee;
                            if (fee) {
                              return (
                                <p className="text-xs text-muted-foreground mt-1">
                                   Fee: {fee} EGP
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {[selectedBranch.area, selectedBranch.city].filter(Boolean).join(', ')}
                    </p>
                  </div>
                </div>
                <Button
                    ...
                    {availability.map((day) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-lg font-semibold">{day.dayNumber} {day.month}</span>
                          {availableSlots > 0 ? (
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="font-medium">
                          Available time slots for {moment(selectedDate).format('D MMM YYYY')}
                        </h4>
                      </div>
                      
                      {selectedDayAvailability.slots.filter(s => s.is_available).length === 0 ? (
                        <div className="text-center py-6 text-muted-foreground bg-muted rounded-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-muted p-4 rounded-lg space-y-2">
              ...
              <p className="text-sm font-medium">
                Time: {formatTime12Hour(selectedSlot?.start_time || '')} - {formatTime12Hour(selectedSlot?.end_time || '')}
              </p>
            </div>
